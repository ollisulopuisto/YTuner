...
def write_png(path, w, h, rows):
    ...
    return png if path is None else len(png)


def png_bytes(w, h, rows):
    return write_png(None, w, h, rows)

def pascal_include(path, w, h):
    """Emit the mark as a Pascal string constant.

    Retuner ships as one binary with no asset directory beside it, so a
    placeholder icon has to live inside the executable. Base64 in an include
    file keeps it reviewable in a diff and regenerable from this script, rather
    than being a blob nobody can account for.
    """
    import base64
    png = png_bytes(w, h, build(w, h))
    b64 = base64.b64encode(png).decode()
    lines = [b64[i:i + 76] for i in range(0, len(b64), 76)]
    with open(path, "w") as f:
        f.write("// Generated by script/make-icon.py --pascal; do not edit by hand.\n")
        f.write("// The Retuner mark, %dx%d, served when a station has no logo of its\n"
                "// own. See GetIcon in httpserver.pas.\n" % (w, h))
        f.write("  PLACEHOLDER_ICON_PNG_BASE64 =\n")
...
def main():
    ...
    ap.add_argument("--out", help="write one file here instead of the defaults")
    ap.add_argument("--size", type=int, nargs=2, metavar=("W", "H"), default=(512, 512))
    ap.add_argument("--pascal", action="store_true",
                    help="emit a Pascal include with the PNG as base64")
    a = ap.parse_args()
    if a.out and a.pascal:
        n = pascal_include(a.out, a.size[0], a.size[1])
        print(f"{a.out}  {a.size[0]}x{a.size[1]}  {n} bytes of PNG, base64 inline")
        return
    if a.out:
        n = write_png(a.out, a.size[0], a.size[1], build(a.size[0], a.size[1]))
        print(f"{a.out}  {a.size[0]}x{a.size[1]}  {n/1024:.1f} KB")
        return
